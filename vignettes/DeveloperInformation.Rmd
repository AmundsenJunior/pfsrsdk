...
The `pfsrsdk` package has sets of unit and acceptance tests. The unit tests,
located in `tests/testthat`, can be executed any time, via `devtools::test()` or
`R CMD check`.

### Acceptance Tests

The acceptance tests (tests that evaluate compatibility with the
requirements and dependencies of the package) are located in
`tests/testthat/acceptance_tests`. These tests are written to use a configurable
set of values for evaluating the SDK calls against a user's development or
automated test environment. This configuration is made in
`tests/testthat/test_environment`, on a corresponding set of `Auth-X.Y.Z.json`
and `Data-X.Y.Z.json` files. Setting values in the `Data-X.Y.Z.json` files that
match data present in a user's development or test environment enable the tests
to execute successfully, regardless of environment or available data.
...
```r
testthat::test_dir("tests/testthat/acceptance_tests", filter = "updateEntityLocation")
```
...
`testthat::test_dir("tests/testthat/acceptance_tests")`|`./gradlew acceptanceTestPackage`
